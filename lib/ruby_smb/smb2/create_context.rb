module RubySMB
  module SMB2
    ...
    class CreateContext < BinData::Record
      ...
      uint16  :name_offset, label: 'Offset to Name/Tag',      initial_value:  lambda { name.rel_offset }
      uint16  :name_length, label: 'Length of Name/Tag',      initial_value:  lambda { name.length }
      uint16  :reserved,    label: 'Reserved Space'
      uint16  :data_offset, label: 'Offset to data',          initial_value:  lambda { calc_data_offset }
      uint32  :data_length, label: 'Length of data',          initial_value:  lambda { data.length }
